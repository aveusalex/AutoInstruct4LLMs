...
from tools import verify_and_create_dir
...
class Denoising:
    ...
    def denoise_nuvem(audio_path: str = None, save_path: str = None, audio_name: str = None) -> None:
        ...
        url = "http://200.137.197.252:5001/denoiser"
    ...
    def denoise(self, audio_path: str = None, save_path: str = None, tensor: torch.tensor = None,
        ...
        if not self.denoiser_model:
            self.denoiser_model = pretrained.dns64().cuda()  # TODO: pq esse modelo em especifico?
        ...
        denoised_audio = torch.tensor([]).cuda()
        with torch.no_grad():
        ...
        if save_path:
            ...
            # de 16000
            verify_and_create_dir(save_path)
            save_path = os.path.join(save_path, audio_name)
            torchaudio.save(save_path, denoised_audio.cpu(), 16000)
        else:
...
def denoisar(audio_name: str = None, dns_object: Denoising = None, mp=False):
    if mp:
        dns_object = Denoising()
    # split dos canais
    audio_cliente, audio_atendente = dns_object.separate_channels(audio_name=audio_name,
                                                                  path_voicerecords="./dados/bronze/anatel")
    # denoise de cada canal separadamente e salva na pasta silver
    dns_object.denoise(tensor=audio_cliente, sample_rate=16000, save_path=f"./dados/silver/anatel/",
                       audio_name=f"{audio_name.replace('.opus', '')}_cliente_denoised.wav")
    dns_object.denoise(tensor=audio_atendente, sample_rate=16000, save_path=f"./dados/silver/anatel/",
                       audio_name=f"{audio_name.replace('.opus', '')}_atendente_denoised.wav")
